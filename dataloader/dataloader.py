...
class HeatmapDataset(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self, stage, root,transform,target_transform): 
        self.stage = stage
        self.root = root
        self.transform = transform
        self.target_transform = target_transform
        if self.stage=='train':
            self.datadir = root+'\\train_data'
            self.labeldir = root+'\\train_label'
        elif self.stage=='val':
    ...
    def __getitem__(self,index): 
        ...
        labelname = self.labeldir + self.filenames[index]
        label = cv2.imread(labelname)[:, :, ::-1].astype(np.uint8)# heatmap
        data = self.transform(data)
        label = self.transform(label)
        return data,label

class HeatmapDataloader(pl.LightningDataModule):
    def __init__(self, batch_size=64):
        super().__init__()
        self.batch_size = batch_size

    def setup(self, stage,root):
        # transform
        transform=transforms.Compose([transforms.ToTensor()])
        # assign to use in dataloaders
        self.train_dataset =  HeatmapDataset(stage,root,transform,target_transform = None)
        self.val_dataset = HeatmapDataset(stage,root,transform,target_transform = None)
        self.test_dataset = HeatmapDataset(stage,root,transform,target_transform = None)
...
class FeaturemapDataloader(pl.LightningDataModule):
    def __init__(self, batch_size=64):
        super().__init__()
        self.batch_size = batch_size
    ...
    def setup(self, stage,root):
        ...
        transform=transforms.Compose([transforms.ToTensor()])
        # assign to use in dataloaders
        self.train_dataset =  FeaturemapDataset(stage,root,transform,target_transform=None)
        self.val_dataset = FeaturemapDataset(stage,root,transform,target_transform=None)
        self.test_dataset = FeaturemapDataset(stage,root,transform,target_transform=None)
