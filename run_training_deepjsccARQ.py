...
from model.model import DeepJSCCARQ,DeepJSCC
...
from model.module.heatmap_predictor import Unet
from model.module.image_cropper import image_cropper
from model.module.policynet import policynet
from model.module.discriminator import pDecoder
from channels.AWGN import AWGNChannel
from dataloader.dataloader import FeaturemapDataloader,SCDataloader
from loss.mixure_loss import MixtureLossImage,BCELossAck,CELossAck,MSEImageLoss
...
def train_feature_scarq(hparams):
    """
    input : hparams
    output: training
    """
    ...
    #define parity_D used to generate ack
    pD = pDecoder()
    ...
    if len(ckpt_addr)==0:
        model = DeepJSCCARQ(
                    ...
                    loss_module_D=BCELossAck(),loss_module_G=MSEImageLoss(),
                    channel=AWGNChannel(),
                    pDecoder = pD,
                    threshold=0.8,hyperparameter=0.05,
                    lr_D = 5e-6,lr_G = 1e-4,lr_scheduler=hparams.lr_scheduler_type,
                    weights_path = str(hparams.e_d_addr),
                    freeze=True
                )
    else:
        model= DeepJSCCARQ.load_from_checkpoint(
            ...
            channel=AWGNChannel(),
            pDecoder = pD,
            threshold=0.8,hyperparameter=0.01,
            lr_D = 5e-5,lr_G = 1e-4,lr_scheduler = hparams.lr_scheduler_type,
            weights_path = str(hparams.e_d_addr),
            freeze=False
            )
    ...
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,shuffle=True, num_workers = 31,pin_memory = True)
    val_loader = torch.utils.data.DataLoader(valset, batch_size=batch_size,shuffle=False, num_workers=31,pin_memory = True)
    # create logger instance
    logger = TensorBoardLogger("logs",name = "deepjsccARQ/")
    # define call backs
    callbacks = [pl.callbacks.ModelCheckpoint(every_n_epochs=int(hparams.save_ckpt_every_n_epochs),save_top_k = 5,monitor="val_loss",mode="min"),
    ...
    callbacks = [pl.callbacks.ModelCheckpoint(every_n_epochs=int(hparams.save_ckpt_every_n_epochs),save_top_k = -1),
                EarlyStopping(monitor = 'val loss',min_delta=0.0005, patience=5)]
    ''' 
    #define profiler:
...
def main(hparams):
    train_feature_scarq(hparams)

if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument("--ckpt_addr", type = str,default='')
    parser.add_argument("--e_d_addr", type = str,default='')
...
#python run_training_deepjscc.py --ckpt_addr '/home/k1928-4/chz/code1/logs/deepjscc/version_8/checkpoints/epoch=395-step=2048000.ckpt' --batch_size 16 --device 1 --max_epoches 1000 --plot_lr True --check_val_every_n_epoch 10 --save_ckpt_every_n_epochs 10 --milestones [250,500,750] --fast_dev_run False
    ...
    #python run_training_deepjsccARQ.py --ckpt_addr '' --e_d_addr './logs/deepjscc/test.ckpt' --batch_size 32 --device 1 --max_epoches 10  --fast_dev_run True --lr_scheduler_type step --find_init_lr False 
    #python run_training_deepjsccARQ.py --ckpt_addr '' --e_d_addr '' --batch_size 48 --device 2 --max_epoches 10 --fast_dev_run True --lr_scheduler_type step --find_init_lr False
#python run_training_deepjsccARQ.py --ckpt_addr '' --e_d_addr 'logs/deepjsccAF/checkpoints/epoch=889-step=347990.ckpt' --batch_size 32 --device 2 --max_epoches 10  --fast_dev_run True --lr_scheduler_type step --find_init_lr False 